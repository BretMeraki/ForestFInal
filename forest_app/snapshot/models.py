...
class UserModel(Base):
    ...
    # MVP simple tasks relationship
    simple_tasks: Mapped[List["UserSimpleTaskModel"]] = relationship("UserSimpleTaskModel", back_populates="user")
...
# --- UserSimpleTask Model (MVP) ---
class UserSimpleTaskModel(Base):
    """Model for the ultra-lean MVP: User Goal -> LLM Task List -> Mark Done"""
    __tablename__ = "user_simple_tasks"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    goal_statement: Mapped[str] = mapped_column(Text, nullable=False)  # The user's original goal
    task_description: Mapped[str] = mapped_column(Text, nullable=False)  # A specific task generated from the goal
    is_done: Mapped[bool] = mapped_column(Boolean, default=False)  # Whether the task has been completed
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
    # Optional sequence number if tasks need to be ordered
    sequence: Mapped[Optional[int]] = mapped_column(Integer, nullable=True) 
    # Store any additional metadata about the task
    metadata: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSONB, nullable=True)

    # --- Relationships ---
    user: Mapped["UserModel"] = relationship("UserModel", back_populates="simple_tasks")
