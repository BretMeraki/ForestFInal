...
class HTAService:
    ...
    def __init__(self, llm_client: Optional[LLMClient] = None, seed_manager: Optional[SeedManager] = None):
        self.tasks = {}
        self.llm_client = llm_client
        self.seed_manager = seed_manager
        self.last_error = None
        self.last_operation_success = True
        self.context_cache = {}  # Add context cache
        logger.debug("HTAService initialized")
    ...
    async def update_node_status(self, tree: HTATree, node_id: str, new_status: str) -> bool:
        ...
        valid_statuses = {"pending", "in_progress", "completed", "pruned"}
        if not isinstance(new_status, str) or new_status.lower() not in valid_statuses:
            logger.error(f"Invalid status value: {new_status}")
            self.last_error = f"Invalid status value: {new_status}"
            self.last_operation_success = False
            increment_hta_failure(f"update_node_status error: Invalid status value: {new_status}")
            return False

        node = tree.find_node_by_id(node_id)
        if node:
            logger.info(f"Updating status for node '{node.title}' ({node_id}) to '{new_status}'.")
            tree.update_node_status(node_id, new_status.lower())  # Convert to lowercase for consistency
            return True
        else:
    ...
    async def evolve_tree(self, tree: HTATree, reflections: List[str], snapshot: Optional[MemorySnapshot] = None, goal: str = None, context: str = None, user_id: int = None) -> Optional[HTATree]:
        ...
        # Get original goal and context from tree or parameters
        original_goal = goal or tree.root.title
        original_context = context or tree.root.description

        # Cache the context for this tree
        tree_id = getattr(tree.root, 'id', None)
        if tree_id:
            self.context_cache[tree_id] = {
                'goal': original_goal,
                'context': original_context
            }

        # Prepare evolution prompt with context preservation
        evolution_prompt = {
            'goal': original_goal,
            'context': original_context,
            'reflections': reflections,
            'preserve_context': True  # Signal to LLM to maintain context
        }

        evolution_goal = (
            f"Previous task batch complete. Re-evaluate the plan and suggest next steps while maintaining the original "
            f"goal: '{original_goal}' and context: '{original_context}'. Ensure all new nodes relate to this context."
        )

        try:
            current_hta_json = json.dumps(tree.to_dict())
            ...
            # Verify root ID and context preservation
            ...
            if llm_root_id != original_root_id:
                ...
                self.last_operation_success = False
                increment_hta_failure(f"evolve_tree error: LLM HTA evolution root ID mismatch")
                raise Exception("LLM evolution root ID mismatch")

            # Verify context preservation in evolved tree
            evolved_root = evolved_hta_response.hta_root
            if not self._verify_context_preservation(evolved_root, original_goal, original_context):
                logger.error("Context not properly preserved in evolved tree")
                self.last_error = "Context not properly preserved in evolved tree"
                self.last_operation_success = False
                increment_hta_failure("evolve_tree error: Context not preserved")
                raise Exception("Context preservation failed")
            ...
            try:
                ...
                if new_tree and new_tree.root:
                    # Verify the evolved tree structure
                    if not self._verify_evolved_tree_structure(new_tree, tree):
                        raise ValueError("Invalid evolved tree structure")
                    ...
                    self.last_operation_success = False
                    increment_hta_failure("evolve_tree error: Failed to re-parse evolved HTA dictionary")
            ...
            logger.exception(f"Error during tree evolution: {e}")
            self.last_error = str(e)
            self.last_operation_success = False
            increment_hta_failure(f"evolve_tree error: {e}")
            
            # Attempt recovery if possible
            if snapshot is not None and goal is not None and context is not None and user_id is not None:
                return await self._attempt_recovery(snapshot, goal, context, user_id, reflections)
            return None

    def _verify_context_preservation(self, node: Any, goal: str, context: str) -> bool:
        """Verify that a node and its children maintain the original context."""
        try:
            # Check node attributes
            if not hasattr(node, 'title') or not hasattr(node, 'description'):
                return False

            # Root node should exactly match goal and contain context
            if getattr(node, 'id', '').startswith('root_'):
                if node.title != goal:
                    return False
                if context.lower() not in node.description.lower():
                    return False

            # Child nodes should relate to goal and context
            else:
                ...
                # Check for minimal context overlap
                if not (goal_words & title_words or goal_words & desc_words):
                    return False
            ...
            return all(self._verify_context_preservation(child, goal, context) for child in children)

        except Exception as e:
            logger.error(f"Error verifying context preservation: {e}")
            return False

    def _verify_evolved_tree_structure(self, new_tree: HTATree, original_tree: HTATree) -> bool:
        """Verify that the evolved tree maintains a valid structure."""
        try:
            # Check root node
            if not new_tree.root or not original_tree.root:
                return False

            # Verify root properties are preserved
            if new_tree.root.id != original_tree.root.id:
                return False

            # Verify tree has grown or maintained size
            new_nodes = len(new_tree.get_node_map())
            original_nodes = len(original_tree.get_node_map())
            if new_nodes < original_nodes:
                return False

            # Verify completed nodes remain completed
            original_completed = {
                node_id for node_id, node in original_tree.get_node_map().items()
                if getattr(node, 'status', '') == 'completed'
            }
            new_completed = {
                node_id for node_id, node in new_tree.get_node_map().items()
                if getattr(node, 'status', '') == 'completed'
            }
            if not original_completed.issubset(new_completed):
                return False

            return True

        except Exception as e:
            logger.error(f"Error verifying evolved tree structure: {e}")
            return False
    ...
    async def _attempt_recovery(
        ...
        """Attempt to recover from evolution failure."""
        try:
            logger.info(f"Attempting recovery for user {user_id}")
            
            # Generate new base tree
            hta_model_dict, _ = await self.generate_onboarding_hta(goal, context, user_id, snapshot)
            if hta_model_dict.get('_fallback_used'):
                logger.warning("Recovery failed: Could not generate new base tree")
                return None

            # Create new tree and evolve it
            base_tree = HTATree.from_dict(hta_model_dict)
            evolved_tree = await self.evolve_tree(base_tree, reflections)
            
            if evolved_tree:
                await self.save_tree(snapshot, evolved_tree)
                logger.info("Recovery successful: Generated and evolved new tree")
                return evolved_tree
            
            logger.warning("Recovery failed: Could not evolve new tree")
            return None
