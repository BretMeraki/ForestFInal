...
class HTANodeModel(BaseModel):
    ...
    magnitude: float = Field(default=5.0, ge=0.0, description="Impact/size score (e.g., 1.0-10.0).")
    ...
    def check_depends_on_not_self(cls, v, info: ValidationInfo):
    ...
    def sync_status_suggestion(cls, v, info: ValidationInfo):
        """Ensure status_suggestion is synced with status if not explicitly set."""
        status_suggestion = info.data.get('status_suggestion')
        if status_suggestion is None:
            info.data['status_suggestion'] = v
        return v
