    ...
    def from_dict(cls, data: dict) -> "HTANode":
        ...
        try:
            ...
            if "title" not in data:
                raise ValueError("Node data missing required 'title' field")
            ...
            try:
                ...
                priority_val = max(0.0, min(1.0, priority_val))  # Clamp between 0.0 and 1.0
            except (ValueError, TypeError):
                logger.warning("Invalid priority value for node %s. Using default.", node_id)
                priority_val = DEFAULT_TASK_PRIORITY

            # Handle magnitude with validation
            try:
                magnitude_val = float(data.get("magnitude", DEFAULT_TASK_MAGNITUDE))
                magnitude_val = max(0.0, magnitude_val)  # Ensure non-negative
            except (ValueError, TypeError):
                logger.warning("Invalid magnitude value for node %s. Using default.", node_id)
            ...
            for child_data in data.get("children", []):
                try:
                    ...
                    logger.error("Error creating child node: %s", child_err)
                    increment_hta_failure(f"Error creating child node: {child_err}")
                    continue  # Skip invalid child but continue with others

            # Handle status fields
            status = data.get("status")
            if status is None:
                status = data.get("status_suggestion", "pending")
            
            # Validate status
            valid_statuses = {"pending", "active", "completed", "pruned", "blocked"}
            if not isinstance(status, str) or status.lower() not in valid_statuses:
                logger.warning("Invalid status '%s' for node %s. Using 'pending'.", status, node_id)
                status = "pending"
            ...
            node = cls(
                ...
                status=status.lower(),  # Ensure lowercase
            ...
            logger.error("Error creating HTANode from dict: %s", e)
            increment_hta_failure(f"Error creating HTANode from dict: {e}")
            raise
