...
class Container(containers.DeclarativeContainer):
    ...
    # Sentiment Engine - Changed to Factory to ensure concurrency safety
    sentiment_engine = providers.Factory(
        SecretSauceSentimentEngineHybrid, llm_client=llm_client
    ) if modules_core_import_ok and llm_import_ok and is_enabled(Feature.SENTIMENT_ANALYSIS) else providers.Factory(DummyService)

    # Pattern Engine - Changed to Factory to ensure concurrency safety
    pattern_engine = providers.Factory(
        PatternIdentificationEngine, config=config.provided
    ) if modules_core_import_ok and is_enabled(Feature.PATTERN_ID) else providers.Factory(DummyService)

    # Practical Consequence Engine - Changed to Factory to ensure concurrency safety
    practical_consequence_engine = providers.Factory(
        PracticalConsequenceEngine
    ) if modules_core_import_ok and is_enabled(Feature.PRACTICAL_CONSEQUENCE) else providers.Factory(DummyService)

    # Metrics Specific Engine - Changed to Factory to ensure concurrency safety
    metrics_engine = providers.Factory(
        MetricsSpecificEngine,
        alpha=config.METRICS_ENGINE_ALPHA,
        thresholds=config.METRICS_ENGINE_THRESHOLDS
    ) if modules_core_import_ok else providers.Factory(DummyService)

    # Seed Manager - Changed to Factory to ensure concurrency safety
    seed_manager = providers.Factory(SeedManager) if modules_core_import_ok else providers.Factory(DummyService)

    # Relational Manager - Changed to Factory to ensure concurrency safety
    relational_manager = providers.Factory(
        RelationalManager, llm_client=llm_client
    ) if modules_core_import_ok and llm_import_ok and is_enabled(Feature.RELATIONAL) else providers.Factory(DummyService)

    # Narrative Modes Engine - Changed to Factory to ensure concurrency safety
    narrative_engine = providers.Factory(
        NarrativeModesEngine,
        config=config.NARRATIVE_ENGINE_CONFIG
    ) if modules_core_import_ok and is_enabled(Feature.NARRATIVE_MODES) else providers.Factory(DummyService)

    # XP Mastery - Changed to Factory to ensure concurrency safety
    xp_mastery = providers.Factory(
        XPMastery
    ) if modules_core_import_ok and is_enabled(Feature.XP_MASTERY) else providers.Factory(DummyService)

    # Emotional Integrity Engine - Changed to Factory to ensure concurrency safety
    emotional_integrity_engine = providers.Factory(
        EmotionalIntegrityIndex, llm_client=llm_client
    ) if modules_core_import_ok and llm_import_ok and is_enabled(Feature.EMOTIONAL_INTEGRITY) else providers.Factory(DummyService)

    # Snapshot Flow Controller - Changed to Factory to ensure concurrency safety
    snapshot_flow_controller = providers.Factory(
        SnapshotFlowController,
        frequency=config.SNAPSHOT_FLOW_FREQUENCY,
        max_snapshots=config.SNAPSHOT_FLOW_MAX_SNAPSHOTS
    ) if modules_core_import_ok else providers.Factory(DummyService)

    # Desire Engine - Changed to Factory to ensure concurrency safety
    desire_engine = providers.Factory(
        DesireEngine, llm_client=llm_client
    ) if modules_core_import_ok and llm_import_ok and is_enabled(Feature.DESIRE_ENGINE) else providers.Factory(DummyService)

    # Financial Readiness Engine - Changed to Factory to ensure concurrency safety
    financial_engine = providers.Factory(
        FinancialReadinessEngine, llm_client=llm_client
    ) if modules_core_import_ok and llm_import_ok and is_enabled(Feature.FINANCIAL_READINESS) else providers.Factory(DummyService)

    # Offering Router - Changed to Factory to ensure concurrency safety
    offering_router = providers.Factory(
        OfferingRouter,
        llm_client=llm_client,
        desire_engine=desire_engine,
        financial_engine=financial_engine
    ) if modules_core_import_ok and llm_import_ok and is_enabled(Feature.REWARDS) else providers.Factory(DummyService)

    # Task Engine - Changed to Factory to ensure concurrency safety
    task_engine = providers.Factory(
        TaskEngine,
        pattern_engine=pattern_engine,
        task_templates=config.TASK_ENGINE_TEMPLATES
    ) if modules_core_import_ok and is_enabled(Feature.CORE_TASK_ENGINE) else providers.Factory(DummyService)

    # Trigger Phrase Handler - Changed to Factory to ensure concurrency safety
    trigger_phrase_handler = providers.Factory(
        TriggerPhraseHandler
    ) if modules_core_import_ok and is_enabled(Feature.TRIGGER_PHRASES) else providers.Factory(DummyService)

    # Harmonic Framework Components - Changed to Factory to ensure concurrency safety
    silent_scorer = providers.Factory(SilentScoring) if modules_core_import_ok else providers.Factory(DummyService)
    harmonic_router = providers.Factory(HarmonicRouting) if modules_core_import_ok else providers.Factory(DummyService)
    ...
    component_state_manager = providers.Factory(
        ComponentStateManager,
        managed_engines=managed_engines_dict
    ) if modules_core_import_ok else providers.Factory(DummyService)

    # HTA Service - Changed to Factory to ensure concurrency safety
    hta_service = providers.Factory(
        HTAService,
        llm_client=llm_client,
        seed_manager=seed_manager,
    ) if modules_core_import_ok else providers.Factory(DummyService)

    # Reflection Processor - Changed to Factory to ensure concurrency safety
    reflection_processor = providers.Factory(
        ReflectionProcessor,
        llm_client=llm_client,
    ...
    # Completion Processor - Changed to Factory to ensure concurrency safety
    completion_processor = providers.Factory(
        CompletionProcessor,
    ...
    # CRITICAL: Changed to Factory to ensure each request gets a fresh instance
    # This prevents state bleeding between concurrent requests
