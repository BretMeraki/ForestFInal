...
=======
        ...
        logger.info(
            f"TaskQueue initialized with {max_workers} workers and {result_ttl}s result TTL"
        )
...
=======
                ...
                logger.debug(
                    f"Worker {worker_id} processing task {task_id} (priority: {priority})"
                )
...
=======
                ...
                if expired_tasks:
                    logger.debug(
                        f"Cleaned up {len(expired_tasks)} expired task results"
                    )
