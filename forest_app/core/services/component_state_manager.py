...
=======
            ...
            # raise TypeError("managed_engines must be a dictionary.")
        logger.info(
            f"ComponentStateManager initialized to manage components with keys: {list(managed_engines.keys())}"
        )
>>>>>>> cede20c (Fix Pylint critical errors: update BaseSettings import for Pydantic v1, ensure dependency_injector and uvicorn are installed)
...
=======
            ...
            if not isinstance(engine_state_dict, dict):
                logger.warning(
                    f"Invalid state format found for component key '{key}'. Expected dict, got {type(engine_state_dict).__name__}. Skipping load."
                )
                continue  # Skip if state isn't a dictionary
            ...
            if (
                ...
                if type(engine).__name__ == "DummyService":
                    logger.debug(
                        f"Skipping state load for component key '{key}' because engine is a DummyService."
                    )
                    continue

                try:
                    logger.debug(
                        f"Loading state for component key '{key}' into {type(engine).__name__}..."
                    )
                    engine.update_from_dict(engine_state_dict)
                except Exception as e:
                    logger.exception(
                        f"Failed to load state for component key '{key}' into {type(engine).__name__}: {e}"
                    )
            elif engine:
                # Engine exists but lacks the method (or is maybe None, though __init__ checks keys)
                logger.debug(
                    f"State found for key '{key}', but engine {type(engine).__name__} lacks 'update_from_dict' method. Skipping load."
                )
>>>>>>> cede20c (Fix Pylint critical errors: update BaseSettings import for Pydantic v1, ensure dependency_injector and uvicorn are installed)
            ...
            if engine and hasattr(engine, "to_dict") and callable(engine.to_dict):
                ...
                if type(engine).__name__ == "DummyService":
                    ...
                    if key in cs:
                        del cs[key]
                        logger.debug(
                            f"Removed potentially stale state for dummy component key '{key}'."
                        )
                    continue
                ...
                try:
                    ...
                    if isinstance(engine_data, dict) and engine_data:
                        ...
                        del cs[key]
                        logger.debug(
                            f"Removed empty/null state for component key '{key}' from {type(engine).__name__}."
                        )

                except Exception as e:
                    logger.exception(
                        f"Failed to get state dictionary using to_dict() for component key '{key}' from {type(engine).__name__}: {e}"
                    )

            elif engine and type(engine).__name__ != "DummyService":
                # Real engine exists but lacks the method
                logger.debug(
                    f"Engine {type(engine).__name__} (key: '{key}') lacks 'to_dict' method. Cannot save its state."
                )
                # else: # Engine instance itself might be None or Dummy
                # logger.debug(f"No valid engine instance or is DummyService for component key '{key}'. Skipping save.")
                pass  # Already handled dummy check above
