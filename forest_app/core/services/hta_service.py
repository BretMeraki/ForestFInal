...
=======
        ...
        if active_seed_id and hasattr(self.semantic_memory_manager, "get_seed_by_id"):
            try:
                ...
                logger.warning(
                    f"Could not get seed by ID {active_seed_id} from snapshot: {e}"
                )
...
=======
        if (
            ...
            logger.debug(
                f"Loading HTA tree from active seed ID: {getattr(active_seed, 'seed_id', 'N/A')}"
            )
            current_hta_dict = active_seed.hta_tree
        # Priority 2: Load from snapshot core_state
...
=======
                    logger.info(
                        f"Successfully loaded HTA tree with root: {tree.root.id} - '{tree.root.title}'"
                    )
                    return tree
                else:
                    logger.error(
...
=======
        if active_seed and hasattr(self.semantic_memory_manager, "update_seed"):
            try:
                ...
                if success:
                    logger.info(
                        f"Successfully updated HTA tree in active seed ID: {active_seed.seed_id}"
                    )
                    seed_save_ok = True
                else:
                    logger.error(
                        f"SeedManager failed to update HTA tree for seed ID: {active_seed.seed_id}"
                    )
            except Exception as seed_update_err:
                logger.exception(
                    f"Failed to update seed {active_seed.seed_id} with final HTA: {seed_update_err}"
                )
        elif not active_seed:
            logger.error("Cannot save HTA to seed: Active seed not found.")
        else:  # SeedManager missing method
...
=======
            ...
            logger.info(
                f"Updating status for node '{node.title}' ({node_id}) to '{new_status}'."
            )
            ...
            logger.warning(
                f"Cannot update status: Node with id '{node_id}' not found in tree."
            )
            return False  # Failure handled gracefully to protect the user experience
...
=======
        ...
        if reflections and hasattr(self.llm_client, "distill_reflections"):
            logger.info(
                f"Distilling {len(reflections)} reflections for evolution goal..."
            )
            try:
                ...
                if distilled_response and distilled_response.distilled_text:
                    evolution_goal = distilled_response.distilled_text
                    logger.info(
                        f"Using distilled reflection as evolution goal: '{evolution_goal[:100]}...'"
                    )
                else:
                    logger.warning(
                ...
                logger.exception(
                    f"Error during reflection distillation: {distill_err}. Using default goal."
                )
>>>>>>> cede20c (Fix Pylint critical errors: update BaseSettings import for Pydantic v1, ensure dependency_injector and uvicorn are installed)
...
=======
            current_hta_json = json.dumps(tree.to_dict())  # Serialize current tree
            logger.debug(
                f"Calling request_hta_evolution. Goal: '{evolution_goal[:100]}...'"
            )
            ...
            if (
                ...
                logger.error(
                    f"Failed to get valid evolved HTA from LLM. Type: {type(evolved_hta_response)}. Response: {log_response}"
                )
                return None  # Evolution failed
            ...
            if llm_root_id != original_root_id:
                logger.error(
                    f"LLM HTA evolution root ID mismatch ('{llm_root_id}' vs '{original_root_id}'). Discarding evolved tree."
                )
                return None  # Evolution result is invalid
...
=======
                ...
                logger.exception(
                    f"Unexpected error parsing evolved HTA dictionary: {parse_err}"
                )
                return None

        except (LLMError, LLMValidationError) as llm_evolve_err:
            logger.error(
                f"LLM/Validation Error during HTA evolution request: {llm_evolve_err}"
            )
            return None  # Evolution failed
        except Exception as evolve_err:
            logger.exception(
                f"Unexpected error during HTA evolution process: {evolve_err}"
            )
            return None  # Evolution failed
...
=======
            self.logger.warning(
                f"Cannot update task {task_id}: Not found in hierarchies"
            )
            return {
                "success": False,
                "message": "We couldn't find this part of your journey. Let's make sure we're on the same path.",
...
=======
        ...
        self.logger.info(
            f"Updated task '{title}' ({task_id}) status from '{old_status}' to '{status}'"
        )
        return result
...
=======
        ...
        if task_id not in self.task_hierarchies:
            self.logger.warning(
                f"Cannot track emotional journey: Task {task_id} not found"
            )
            return
...
=======
        ...
        if streak >= 3:
            enhancements.append(
                f"You're on a {streak}-day streak! Consistency is transformative."
            )
        ...
        if velocity >= 3:
            enhancements.append(
                f"You're moving at an impressive pace of {velocity} completions per week."
            )
...
=======
                    ...
                    self.logger.error(
                        f"Failed to record meaningful step completion: {e}"
                    )
