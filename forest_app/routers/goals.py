    ...
    try:
        ...
        try:
            ...
            if not seed_to_complete:
                raise HTTPException(
                    status_code=404, detail=f"Goal ID '{seed_id}' not found."
                )
        ...
        saved_model = await save_snapshot_with_codename(
            db, repo, user_id, snapshot, stored_model
        )
        if not saved_model:
            raise HTTPException(status_code=500, detail="Failed save session.")
        ...
        logger.exception(
            f"DB/Data error /confirm_goal user {user_id} seed {seed_id}: {db_val_err}"
        )
        detail = (
            "DB error."
