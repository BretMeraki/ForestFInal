...
try:
    ...
    logger.debug("Forest Streamlit app started. User session: %s", st.session_state.get('user_info', {}))
    ...
    # --- Developer Tools Link (visible only to admins) ---
    DEV_SUITE_URL = "https://your-debug-suite-name-yourusername.streamlit.app"  # <-- Replace with your actual debug suite Streamlit URL
    if st.session_state.get('authenticated') and st.session_state.get('user_info', {}).get('is_admin', False):
        st.sidebar.markdown("---")
        st.sidebar.markdown(
            f"<a href='{DEV_SUITE_URL}' target='_blank' style='color:#d7263d;font-weight:bold;'>🐞 Developer Tools (Debug Suite)</a>", unsafe_allow_html=True
        )
    ...
    def fetch_hta_state():
        ...
        logger.debug(f"HTA API response: status={status_code}, error={error_msg}, data={str(hta_data)[:200]}")
        if error_msg:
            logger.error(f"HTA fetch error: {error_msg} | Status: {status_code} | Token: {st.session_state.get('token')}")
            st.session_state[KEY_HTA_STATE] = None
            st.session_state[KEY_ERROR_MESSAGE] = f"HTA fetch error: {error_msg}"
            return
        if status_code == 200 and isinstance(hta_data, dict):
            st.session_state[KEY_HTA_STATE] = hta_data
            logger.info("HTA state fetched successfully.")
            ...
            if current_status == constants.ONBOARDING_STATUS_COMPLETED:
                ...
                if prompt := st.chat_input(input_placeholder, disabled=chat_disabled, key="main_chat"):
                    ...
                    with st.chat_message("assistant"):
                        ...
                        logger.info(f"User command: {prompt}")
                        ...
                        logger.debug(f"Command API response: {response}")
                        if response.get(KEY_ERROR):
                            logger.warning(f"Error {api_endpoint}: {response.get(KEY_ERROR)} (Status: {response.get(KEY_STATUS_CODE)})")
                            assistant_response_content = f"⚠️ Error: {response.get(KEY_ERROR)}"
    ...
    def complete_node(node_id):
        logger.info(f"User attempting to complete node: {node_id}")
        response = call_forest_api(
            "/core/complete_task",
            method="POST",
        ...
        logger.debug(f"Complete node response: {response}")
        # Handle 404 or 'Task not found in backlog' gracefully
        if response.get("error"):
            error_msg = str(response["error"])
            logger.warning(f"Complete node error: {error_msg} | node_id={node_id} | status_code={response.get('status_code')}")
        ...
        if isinstance(result, dict) and "tasks" in result:
            logger.info(f"Node {node_id} completed. Tasks returned: {len(result['tasks'])}")
            return result["tasks"]
        logger.info(f"Node {node_id} completed. No tasks returned.")
        return []
